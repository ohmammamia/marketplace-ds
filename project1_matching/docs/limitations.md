...
1. **Logged-policy bias.** Outcomes exist only for pairs the legacy rule chose to notify
   ...
   the offline uplift.
2. **Modest uplift.** +5.5 NDCG points over legacy order, +1.8 over a hand rule. Within
...
5. **Synthetic data.** Effect sizes are properties of the generator, not of the market.
   Numbers here are reproducible only on the pinned dependency versions: ranking ties,
   tree ensembles and the pickled model all shift across library releases.
