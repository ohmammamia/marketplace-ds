...
shrunk towards the global prior so that new providers are not penalised
...
def build_training_set(leads: pd.DataFrame, ins: pd.DataFrame, offers: pd.DataFrame) -> pd.DataFrame:
    ...
    prior = offers.purchased.mean()
    med_price = ins.price_per_hour.median()
    ins_idx = ins.set_index("provider_id")
    rows = []
    # history is recomputed per week to keep it cheap yet leakage-free
